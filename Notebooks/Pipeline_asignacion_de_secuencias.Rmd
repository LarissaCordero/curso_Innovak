...
# 9103 secuencias e identifico  7339/9103
#basados en esto el 80% de mis secuencias son quimeras
...
#tomando en cuenta abundancias en realidad mantuvimos 28% de nuestras lecturas
...
```{r}
out <- read.csv("~/capR/curso/curso_Innovak/Material_clase/Conteo_reads3.csv")
...
track <- cbind(out, # Paso1: filtrado y corte
               sapply(dadaFs_nopool, getN), #paso2:calculamos errores dentro de dada
               sapply(dadaRs_nopool, getN),# paso3: denoising
               sapply(mergers, getN),#paso4: unir muestras
...
write.csv(track, "~/capR/curso/curso_Innovak/Material_clase/Seguimiento_dada.csv") # para guardar una tabla
...
Si inspeccionan la taxonomia asignada y ven que existen muchos errores. Por ejemplo que las secuencias sean asignadas como Eukaryota NA NA NA NA NA NA, las lecturas pueden estar en sentido opuesto a los de la base de referencia. Pueden intentar asignar taxonomia usando orientacion opuesta agregando assignTaxonomy(..., tryRC =TRUE)


## Añadir especies

Auinque al nivel que van a trabajar no va ser tan necesario , DADA2 tmbien implementa un metodos de asginacion de especies basado en alineacion exacta entre nuestros ASVs y las secuencias de referencia, basado en  analisis previos, Sin embargo es importante notar que esta funcion solo funciona con SIlva y RDP
...
Como es de esperarse casi no van a haber identificaciones de especies pero tome en cuenta que es una opcion posible
